...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  FutureOr<void> authlogin(
    ...
    try {
      ...
      print(e.toString());
      emit(const AuthFailureState(failureMsg: AppContsTexts.socketExeption));
    } catch (e) {
      print(e.toString());
      emit(const AuthFailureState(failureMsg: AppContsTexts.socketExeption));
    }
  ...
  FutureOr<void> authForgetPwd(
    ...
    try {
      ...
      res.fold(
          (failure) => emit(AuthForgetPwdFailure(failureMsg: failure.message)),
          (response) {
        if (response['emsg'] == "") {
          emit(const AuthForgetPwdSuccess());
        } else {
          emit(AuthForgetPwdFailure(failureMsg: response['emsg']));
        }
      });
    } on SocketException catch (_) {
      emit(
          const AuthForgetPwdFailure(failureMsg: AppContsTexts.internetFailed));
    } catch (e) {
      ;
      emit(
          const AuthForgetPwdFailure(failureMsg: AppContsTexts.internetFailed));
